...
class EnterpriseCodeServer {
  ...
  private setupToolHandlers(server: McpServer) {
    ...
    server.registerTool("fetch_file_from_git", {
    ...
    }, async({ org, repository, filePath }: { org: string, repository: string, filePath: string }) => {
    ...
    server.registerTool("search_code_embeddings", {
      description: 'Searches the Qdrant code embeddings collection for semantically similar code snippets.',
      inputSchema: z.object({
        org: z.string().describe('GitHub organization name to filter results.'),
        keywords: z.string().describe('Search query or keywords to find relevant code embeddings.'),
      }).required(),
    }, async({ org, keywords }: { org: string, keywords: string }) => {
